...
Debugar () {                                                # Função Debugar
    [ $1 -le $NIVEL_DEBUG ] && echo "Debug $*"              # Se $1 for menor ou igual a $NIVEL_DEBUG, printa "Debug (e os valores passados nas linhas 49 e 51)"
}
...
Soma () {
    ...
    for i in $(seq 1 25); do                                # Enquanto i estiver na sequencia de 1 a 25, faça..
        Debugar 1 "Entrei no for com o valor: $i"           # Puxando a função Debugar, e passando para ela os parametros: 1 e o texto seguinte 
        total=$(($total+$i))                                # Atribuindo a variável total, o valor que está sendo somado a cada iteração
        Debugar 2 "Depois da soma: $total"                  # Puxando a função Debugar, e passando para ela os parametros: 2 e o texto seguinte
    done
